...
module.exports = {
  name: "blackbox",
  description: "Blackbox AI Text Processing",
  options: {
    withoutPrefix: true,
  },
  menu: {
  ...
  run: async ({ m, sock }) => {
    ...
    if (!m.arg) {
      ...
      return sock.sendMessage(m.from, { text: "*Example:* .bx how to attract a woman's heart" }, { quoted: m });
    }

    const skizoApi = apis.sazumiviki;
    const apiUrl = `https://skizo.tech/api/blackbox?text=${encodeURIComponent(m.arg)}&apikey=${skizoApi.apikey}`;
